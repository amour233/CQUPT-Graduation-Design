...
def get_weibo_comment_list(v_weibo_id, v_max_page):
    ...
    while page <= v_max_page:
        ...
        source_list = []  # IP归属地
        user_name_list = []  # 评论用户
        user_follow_list = [] #关注数
        user_follows_list = [] #粉丝数
        user_vip_list = [] #是否会员
        user_gender_list = [] #性别
        for data in datas:
            ...
            user_follow_list.append(data["user"]["follow_count"]) # 关注数
            user_follows_list.append(data["user"]["followers_count"]) # 粉丝数
            user_vip_list.append(data["user"]["svip"]) # svip
            user_gender_list.append(data["user"]["gender"]) # 性别
        df = pd.DataFrame(
            {
                ...
                "评论时间": time_list,
                "IP归属地": source_list,
                "关注数": user_follow_list,
                "粉丝数": user_follows_list,
                "svip": user_vip_list,
                "性别": user_gender_list,
            }
        )
...
def get_weibo_list(v_keyword, v_max_page):
    ...
    for page in range(1, v_max_page + 1):
        ...
        # 作者id
        author_list = jsonpath(cards, "$..mblog.user.id")
        # 是否会员
        vip_list = jsonpath(cards, "$..mblog.user.svip")
        # 转发数
        ...
        # 关注数
        follow_list = jsonpath(cards, "$..mblog.user.follow_count")
        # 粉丝数
        follows_count = jsonpath(cards, "$..mblog.user.followers_count")
        # 性别
