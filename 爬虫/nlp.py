...
def gender_sentiments(df):
    """
    计算性别的情感倾向
    :param df: 表
    :return:
    """
    return df.groupby("性别")["情感倾向"].mean().reset_index().rename(columns={"情感倾向": "平均情感倾向"})

def svip_sentiments(df):
    """
    计算svip的情感倾向
    :param df: 表
    :return:
    """
    return df.groupby("svip")["情感倾向"].mean().reset_index().rename(columns={"情感倾向": "平均情感倾向"})
